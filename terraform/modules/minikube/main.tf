locals {
  ...
  user_data = <<-SCRIPT
    ...
    # Jira, SonarQube, Artifactory, Harbor, ArgoCD, RHBK, Woodpecker) needs to resolve, for any caller running
...
data:
  Corefile: |
    .:53 {
        ...
        rewrite name exact harbor.devopstashtiot.page ingress-nginx-controller.ingress-nginx.svc.cluster.local answer auto
